module ModuleFieldCommunication
    ...
    integer(4), parameter :: field_com_type_nonblock  = 0
    integer(4), parameter :: field_com_type_onesided  = 1

    integer(4), parameter :: field_com_opt_sum        = 0
    integer(4), parameter :: field_com_opt_ext        = 1

    integer(4), parameter :: negb_ext_type_intep      = 0
    integer(4), parameter :: negb_ext_type_symmetry   = 1
    ...
    type :: FieldCom2D
        ...
        integer(4) :: left_ext_type   = negb_ext_type_intep
        integer(4) :: right_ext_type  = negb_ext_type_intep
        integer(4) :: bottom_ext_type = negb_ext_type_intep
        integer(4) :: top_ext_type    = negb_ext_type_intep
    ...
    contains
        ...
        subroutine initFieldCom2D(this, x_local_size, y_local_size, deps_x, deps_y, com_type)
            ...
            if (x_local_size > 0 .and. y_local_size > 0 .and. deps_x > 0 .and. deps_y > 0) then
                ...
                this%dpx = deps_x
                this%dpy = deps_y
                this%com_type = field_com_type_nonblock

                if (present(com_type)) then
                    if (com_type == field_com_type_nonblock) this%com_type = field_com_type_nonblock
                    if (com_type == field_com_type_onesided) this%com_type = field_com_type_onesided
                end if
                ...
                if (this%row - 1 >= 0 .and. this%col - 1 >= 0) then                 ! pos 1
                    this%negb_rank(negb_left_bottom) = (this%row-1) * this%dpx + this%col - 1
                end if

                if (this%row - 1 >= 0) then                                         ! pos 2
                    this%negb_rank(negb_bottom) = (this%row-1) * this%dpx + this%col
                end if

                if (this%row - 1 >= 0 .and. this%col + 1 < this%dpx) then          ! pos 3
                    this%negb_rank(negb_right_bottom) = (this%row-1) * this%dpx + this%col + 1
                end if

                if (this%col + 1 < this%dpx) then                                  ! pos 4
                    this%negb_rank(negb_right) = this%row * this%dpx + this%col + 1
                end if

                if (this%row + 1 < this%dpy .and. this%col + 1 < this%dpx) then   ! pos 5
                    this%negb_rank(negb_right_top) = (this%row+1) * this%dpx + this%col + 1
                end if

                if (this%row + 1 < this%dpy) then                                  ! pos 6
                    this%negb_rank(negb_top) = (this%row+1) * this%dpx + this%col
                end if

                if (this%row + 1 < this%dpy .and. this%col - 1 >= 0) then          ! pos 7
                    this%negb_rank(negb_left_top) = (this%row+1) * this%dpx + this%col - 1
                end if

                if (this%col - 1 >= 0) then                                         ! pos 8
                    this%negb_rank(negb_left) = this%row * this%dpx + this%col - 1
                end if
                ...
                ! 创建远程访问窗口
                if (field_com_type_onesided == this%com_type) then
        ...
        subroutine destroyFieldCom2D(this)
            ...
            if (field_com_type_onesided == this%com_type .and. this%is_init) then
        ...
        subroutine communicationFieldCom2D(this, array2d, opt_type, xstart, xend, ystart, yend)
            ...
            if (this%is_init) then
                ...
                if (field_com_opt_sum == opt_type) then
                    if (field_com_type_nonblock == this%com_type) then
                ...
                else if (field_com_opt_ext == opt_type) then
                    if (field_com_type_nonblock == this%com_type) then
